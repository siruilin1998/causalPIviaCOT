...
from utilities_prop import generate_data_prop, cot_estimator_prop
...
def main_cp(N, tvc, SIMSIZE=120, gap=True):
    ...
    for k in range(SIMSIZE):
        # generate raw data 
        A0, A1, data = generate_data_prop(N, b0, b1, e_z)
...
# plt.ylim(-0.01, 0.17)
plt.legend()
# plt.savefig("prop500error_well_error.pdf")
plt.show()
